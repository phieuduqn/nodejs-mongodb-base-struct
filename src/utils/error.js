...
function parseResponse(paymentType, status, message, amount, realAmount) {
  let msg = message;
  if (
    paymentType === "cashInMobileCard" ||
    paymentType === "checkoutMobileCard"
  ) {
...
function messageMobileCard(errorCode, msg) {
  ...
  if (parseInt(errorCode, 10) === -100)
    return "Sai giá trị mệnh giá thẻ truyền lên";
  if (parseInt(errorCode, 10) === -1)
    return "Sai mã thẻ hoặc thẻ đã được sử dụng";
  return msg;
}
